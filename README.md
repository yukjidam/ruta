...
* [ ] Migrate `plan_ride_screen.dart` and `live_screen.dart` from
  `flutter_map` to **MapLibre**.
...
* New Settings screen, reachable from Profile — grouped account/
  preferences/support rows and an "About Ruta" modal (maker credit, live-
  fetched GitHub avatar, link to
...
* A real app icon: the "R" mark traced as an actual vector outline from
  ...
  as `assets/icon/ruta_icon.{svg,png,ico}`. Used on the login screen, in the
  About modal, and ready to become the real Android/iOS launcher icon
...
Replace the gradient placeholders on Plan a Ride and Live Ride with a real
`flutter_map` + OpenStreetMap view. No account or backend needed for this
step, which is why it's next.
...
Full setup (Flutter requirements, environment variables, git, CI) lives in
